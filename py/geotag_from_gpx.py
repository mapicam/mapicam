...
from lib.geo import interpolate_lat_lon, decimal_to_dms, utc_to_localtime, offset_bearing
...
def get_lat_lon_time(gpx_file):
    ...
    --
    Add: use speed from gpx 1.0 to mark image without motion
    --
    GPX stores time in UTC, assume your camera used the local
    timezone and convert accordingly.
    '''
    GPS_Logger = "BasicAirData GPS Logger 2.1.12"
    GPS_Track_editor = "GPS Track Editor"
    OSMTracker = "OSMTracker for Android™ - https://github.com/labexp/osmtracker-android"
        ...
        for track in gpx.tracks:
            for segment in track.segments:
                for point in segment.points:
                    if version == r'1.0' and creator == GPS_Logger:
                        points.append( (utc_to_localtime(point.time), point.latitude, point.longitude, point.elevation,point.speed) )
                    elif version == r'1.1' and creator == GPS_Track_editor:
                        if len(point.extensions)>0:
                            point.speed = (float(point.extensions[0].attrib['speed']) * 1000) / 3600
                        else:
                            point.speed = None
                        points.append( (utc_to_localtime(point.time), point.latitude, point.longitude, point.elevation,point.speed) )
                    elif version == r'1.1' and creator == OSMTracker:
                        if point.extensions[0].tag == 'speed':
                            point.speed = float(point.extensions[0].text)
                        else:
                            point.speed = None
                        points.append( (utc_to_localtime(point.time), point.latitude, point.longitude, point.elevation,point.speed) )
    ...
    if len(gpx.waypoints) > 0:
        for point in gpx.waypoints:
            points.append( (utc_to_localtime(point.time), point.latitude, point.longitude, point.elevation,None) )
...
def add_exif_using_timestamp(filename, time, points, offset_time=0, offset_angle=0):
    '''
    ...
    try:
        ...
        metadata.write()
        print("Added geodata to {}: {}, {:.6f}, {:.6f}, {:.1f}".format(os.path.basename(filename), time, lat, lon, bearing))
...
def estimate_sub_second_time(files, interval):
    ...
    if smin > smax:
        ...
        return [smin + T * i for i in range(len(files))]
...
def get_args():
    ...
    p.add_argument('--offset_angle',
        help='Angle of the camera, 0 forward in degrees',
        type=int, default=0)
    return p.parse_args()
...
if __name__ == '__main__':
    ...
    for filepath, filetime in zip(file_list, sub_second_times):
        add_exif_using_timestamp(filepath, filetime, gpx, args.time_offset, args.offset_angle)
